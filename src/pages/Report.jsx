...
import { FileText, Printer, ChevronDown, User, BookOpen, Award, GraduationCap } from 'lucide-react';
        ...
        <tbody>
          {gradeBoundaries.map(b => (
            <tr key={b.grade} className="even:bg-gray-50">
              ...
              <td className="px-3 py-2 border border-gray-200 text-gray-600 italic">{b.label}</td>
            </tr>
          ))}
        </tbody>
      </table>
...
function SubjectDetailPage({ subject, grade, criteriaDescriptors, gradeLevel, studentName, nis, studentClass, semester, academicYear, calculateGradeDynamic, getLocalGrade, getScaledBoundaries, pageNum, totalPages }) {
  ...
  return (
    <ReportPage pageNum={pageNum} totalPages={totalPages} studentName={studentName} nis={nis} studentClass={studentClass} semester={semester} academicYear={academicYear}>
      ...
      {/* Criteria sections */}
      {criteriaList.map(c => {
        const desc = descriptors[c] || { title: c, descriptors: [] };
        ...
        return (
          <div key={c} className="mb-5">
            ...
            {desc.descriptors && desc.descriptors.length > 0 && (
              <ul className="list-disc list-inside text-xs text-gray-500 leading-5 space-y-0.5">
                {desc.descriptors.map((d, i) => (
                  <li key={i}>{d}</li>
                ))}
              </ul>
            )}
...
export default function Report() {
  const {
    ...
    calculateGrade, calculateGradeDynamic, getLocalGrade, gradeBoundaries, getScaledBoundaries,
