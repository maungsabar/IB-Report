...
export const ROLE_ACCESS = {
  admin: ['*'], // all pages
  coordinator: ['/', '/master/mata-pelajaran', '/master/deskriptor', '/laporan/rapor'],
  homeroom: ['/', '/akademik/nilai', '/akademik/kehadiran', '/laporan/rapor'],
...
export function AppProvider({ children }) {
  ...
  const [attendances, setAttendances] = useState(data.attendances);
  const [homeroomComments, setHomeroomComments] = useState(data.homeroomComments);
  const [academicYears, setAcademicYears] = useState(data.academicYears);
  const [schoolProfile, setSchoolProfile] = useState(data.schoolProfile);
  const [users, setUsers] = useState(data.users);
  const [alumni, setAlumni] = useState(data.alumni);
  const [classes, setClasses] = useState(data.classes);
  const [criteriaDescriptors, setCriteriaDescriptors] = useState(data.criteriaDescriptors);
  const [learnerProfiles, setLearnerProfiles] = useState(data.learnerProfiles);
  const [ibProfileLogo, setIbProfileLogo] = useState(data.ibProfileLogo);
  const [loading, setLoading] = useState(true);
  const [apiConnected, setApiConnected] = useState(null); // null = belum dicek, true/false
