...
router.get('/:subjectId', async (req, res) => {
  try {
    ...
    // Build nested object: { "7": { "A": { title, descriptors }, "B": ... }, "8": ... }
    const result = {};
...
router.put('/:subjectId', async (req, res) => {
  try {
    ...
    for (const row of rows) {
      if (!result[row.gradeLevel]) result[row.gradeLevel] = {};
      result[row.gradeLevel][row.criteria] = { title: row.title, descriptors: row.descriptors };
    }
    res.json(result);
  } catch (err) {
    res.status(500).json({ error: err.message });
